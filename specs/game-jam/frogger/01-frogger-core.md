...
> **Estado:** Implementado
> **Depende de:** 07-leaderboard-y-tabla-juegos
> **Fecha:** 2026-05-20
...
> Nota general: varios ítems se verifican contra la arquitectura real del repo, no contra los
> nombres literales del spec (ver «Adaptaciones de implementación» al final). Donde aplica, el
> ítem queda marcado igual porque el comportamiento descrito sí se cumple, solo cambia el nombre
> de archivo/ruta/callback.

- [x] La fila `frogger` existe en la tabla `games` de Supabase con los valores del data model. » título/short/long iguales; cover `cover-rana` y color `green` en vez de `cover-frogger`/`lime` (fila reusada de `ranaria`, ver adaptaciones).
- [x] La card de Frogger aparece en `/games` con cover `cover-frogger` y color `lime`. » ruta real `/biblioteca`, cover `cover-rana`, color `green`.
- [x] La ruta `/games/frogger/play` carga sin errores de SSR ni de TypeScript. » ruta real `/jugar/frogger` (no hay play-page dedicada, ver adaptaciones); build sin errores.
...
- [x] Al morir, `onLivesChange(lives - 1)` se dispara; la rana vuelve a la fila de inicio. » traducido a `EngineSnapshot.lives` vía `onSnapshot`, mismo patrón que Asteroides/Arkanoid/Culebra.
...
- [x] `onLevelChange(level)` se dispara al iniciar cada nueva ronda. » traducido a `EngineSnapshot.level`.
- [x] La velocidad de entidades aumenta con cada nivel.
- [x] El temporizador de ronda disminuye con cada nivel.
- [x] `onScoreChange(score)` se dispara en cada cambio de puntuación. » traducido a `EngineSnapshot.score`.
...
- [x] Las teclas P / Esc no provocan una pausa independiente del canvas. » el canvas no engancha ninguna tecla fuera de las flechas.
- [x] Al llegar a `lives = 0`, `onLivesChange(0)` y `onGameOver(score)` se disparan; aparece el modal React. » vía `snapshot.state === "gameover"`.
- [ ] El modal pre-rellena el nombre desde `av_player_name` si existe en localStorage. » **no implementado**: el modal compartido de `jugar-client.tsx` prellena desde la sesión en memoria (`useSession`), no desde `localStorage` — ningún juego de la plataforma usa `localStorage` hoy (`grep` vacío en todo `app/`). Implementarlo tocaría el modal compartido de los 5 juegos, fuera del alcance de "agregar Frogger".
- [ ] Al confirmar el nombre, el score se inserta en Supabase y el nombre se persiste en localStorage. » el insert en Supabase sí ocurre (`saveScore`); la persistencia en `localStorage` no, mismo motivo que el ítem anterior.
- [x] El botón de guardar se deshabilita tras el primer envío (sin doble inserción). » tras guardar, el bloque input+botón se reemplaza por el toast "PUNTUACIÓN GUARDADA" (mismo mecanismo que los otros 4 juegos), no hay forma de reenviar.
- [x] El botón "JUGAR DE NUEVO" reinicia la partida desde cero (nuevo `gameKey`). » traducido al patrón de `ref.restart()` + reset de estado React que ya usan los otros 4 juegos, sin `gameKey`.
- [x] El score guardado aparece en `/games/frogger` y en `/hall-of-fame` al recargar. » rutas reales `/juego/frogger` y `/salon`; ambas se corrigieron durante la Fase 3 para incluir a `frogger` en sus listas de "juego real" (antes caían en datos mock `seededScores`).
- [x] `npm run build` completa sin errores de TypeScript.
- [x] Ninguna ruta existente devuelve 500. » verificado por build; sin smoke-test HTTP de cada ruta en runtime.
...
## Adaptaciones de implementación

Este spec se escribió para una arquitectura de repo (`components/games/`, rutas `/games/*`,
`/hall-of-fame`, callbacks `onScoreChange`/`onLivesChange`/`onLevelChange`/`onGameOver`) distinta
de la que realmente tiene Arcade Vault. Decisiones tomadas durante `/spec-impl-game` para
resolver esa discrepancia, acordadas con el usuario en el momento:

- **Arquitectura real en vez de la literal del spec** — `app/games/frogger/engine.ts` (simulación
  pura) + `frogger-canvas.tsx` (canvas + input), wireados en `app/jugar/[id]/jugar-client.tsx` por
  `game.id === "frogger"`, igual que Asteroides/Tetris/Arkanoid/Culebra. Sin
  `components/games/FroggerGame.tsx` ni `app/games/frogger/play/page.tsx` — esas rutas no existen
  en este proyecto (rutas reales: `/biblioteca`, `/juego/[id]`, `/jugar/[id]`, `/salon`). Los 4
  callbacks del spec se tradujeron a un único `EngineSnapshot { score, lives, level, state }` vía
  `onSnapshot`, el patrón que ya usan los otros 4 juegos.
- **Fila de Supabase reusada, no insertada** — ya existía una fila `ranaria` (placeholder de este
  mismo concepto de juego, cover `cover-rana`, color `green`). Se hizo `UPDATE` de su `id` a
  `frogger` en vez de `INSERT` de una fila nueva, para no duplicar el juego en el catálogo. Se
  mantuvieron `cover-rana` (ya temáticamente correcta) y `color: green` en vez de `cover-frogger`/
  `lime` — este último no es válido en `GameColor` (`app/data/games.ts`: solo
  `cyan`/`magenta`/`green`/`yellow`).
- **Unidades de velocidad de carriles** — el spec da rangos en px/frame asumiendo ~60fps fijo;
  el motor usa `dt` en segundos como el resto de la plataforma, así que las velocidades de
  `buildLanes()` están en columnas/segundo, no en px/frame. Mismo comportamiento (rangos,
  crecimiento 15%/nivel), solo cambian las unidades.
- **Sin `skins.ts` en el paso de `/spec-impl`** — se agregó después, en la Fase 3 de
  `/spec-impl-game`, a cargo de `skin-designer` (`clasico`/`neon`/`retro`), no como parte del plan
  de este spec.
- **Sin persistencia en `localStorage`** — ningún juego de la plataforma usa `localStorage` hoy
  (el modal de guardar score de `jugar-client.tsx` prellena desde `useSession`, en memoria). Los
  dos criterios de aceptación que lo pedían quedaron sin marcar; implementarlo tocaría el modal
  compartido de los 5 juegos, no solo Frogger.
- **`/salon` y `/juego/[id]` actualizados** — tenían listas hardcodeadas de "juegos reales" sin
  `frogger`; se corrigieron para que el leaderboard real (no `seededScores`) se vea también ahí.